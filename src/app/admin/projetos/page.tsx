...
export default function AdminProjetos() {
  ...
  async function load() {
    setLoading(true);
    try {
      const data = await apiWithAuth<Project[]>("/projects");
      setProjetos(data);
    } catch {
      setProjetos([]);
    } finally {
  ...
  async function handleDelete(id: string) {
    ...
    await apiWithAuth(`/projects/${id}`, { method: "DELETE" });
    await load();
    router.refresh();
  }
      ...
      <header className="mb-12 border-b border-zinc-900 pb-8 flex flex-col md:flex-row md:justify-between md:items-end gap-6">
        <div>
          ...
          <p className="text-zinc-500 font-medium">
            Você tem {projetos.length} projetos publicados.
          </p>
        </div>
        <Link
          href="/admin/projetos/novo"
          className="flex items-center gap-2 bg-white text-black px-6 py-4 rounded-lg font-bold uppercase tracking-widest text-xs hover:bg-gray-200 transition-colors shadow-lg"
        >
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-zinc-900 bg-zinc-900/20">
              <th className="p-6 text-xs font-bold uppercase tracking-widest text-zinc-500">
                Projeto
              </th>
              <th className="p-6 text-xs font-bold uppercase tracking-widest text-zinc-500 hidden md:table-cell">
                Slug
              </th>
              <th className="p-6 text-xs font-bold uppercase tracking-widest text-zinc-500 text-right">
                Ações
              </th>
            </tr>
          </thead>
          <tbody>
            {projetos.map((projeto) => (
              ...
              >
                ...
                <td className="p-6 text-zinc-400 text-sm hidden md:table-cell">
                  /{projeto.slug}
                </td>
